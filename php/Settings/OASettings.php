...
class OASettings {
	...
	public function __construct() {
		...
		register_uninstall_hook(OA_PLUGIN_FILE, array($this, 'destroy_settings'));
	}
	...
	public function enqueue_media_scripts(string $hook_suffix) {
		if ($hook_suffix === 'toplevel_page_' . $this->settings_page_name) {
			wp_enqueue_media();
			wp_enqueue_script('oa-media-input-scripts', OA_PLUGIN_URL . 'php/Settings/oa-media-input.js', array('jquery', 'wp-i18n'), OA_PLUGIN_VERSION, true);
			wp_set_script_translations('oa-media-input-scripts', 'order-arena');
		}
	}
	...
	public function render_settings_page_template() {
		...
		if (isset($_GET['settings-updated'])) {
			add_settings_error($this->option_name, 'oa_updated', __('Settings updated!', 'order-arena'), 'success');
		}
