...
class Pages {
	...
	public static function register() {
		...
		register_activation_hook(OA_PLUGIN_FILE, array(static::class, 'register_pages'));
		register_deactivation_hook(OA_PLUGIN_FILE, array(static::class, 'unregister_pages'));
	}
	...
	public static function register_page_type() {
		register_post_type('order_arena_page', array(
			'labels' => array(
				'description' => __('Pages from Order Arena Plugin'),
				'name' => __('Order Arena Pages'),
				'singular_name' => __('Order Arena Page'),
				'add_new' => __('Add New Order Arena Page'),
				'add_new_item' => __('Add New Order Arena Page'),
				'edit_item' => __('Edit Order Arena Page'),
				'new_item' => __('New Order Arena Page'),
				'view_item' => __('View Order Arena Page'),
				'view_items' => __('View Order Arena Pages'),
				'search_items' => __('Search Ordar Arena Pages'),
				'all_items' => __('All Order Arena Pages'),
				'archives' => __('Order Arena Pages'),
				'attributes' => __('Order Arena Page Attributes'),
				'insert_into_item' => __('Insert into Order Arena Page'),
				'uploaded_to_this_item' => __('Uploaded to this Order Arena Page'),
				'featured_image' => __('Order Arena Page Image'),
				'remove_featured_image' => __('Remove Order Arena Page Image'),
				'use_featured_image' => __('Use Order Arena Page Image'),
			),
			'has_archive' => true,
			'rewrite' => array(
				'slug' => 'oa',
	...
	public static function register_pages() {
		foreach (self::$pages as $page) {
			...
			if (!$post || $post->page_template != $page_template_slug || $post->post_type != 'page' || $post->post_status != 'publish') {
				$new_post = self::register_page($page, $page_template_slug, $post_id);

				if (!is_admin() && $new_post) {
	...
	public static function reregister_pages() {
		self::unregister_pages();
		self::register_pages();
		flush_rewrite_rules();
	}
	...
	private static function page_template_exists($page_template_slug) {
		return $page_template_slug && file_exists(OA_PLUGIN_DIR . 'php/Templates/' . $page_template_slug);
	}

	private static function register_page($page_name, $page_template_slug, $post_id = null) {
