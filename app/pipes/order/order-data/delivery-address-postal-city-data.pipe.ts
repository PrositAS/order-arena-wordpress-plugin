...
@Pipe({
  name: 'deliveryAddressPostalCityData',
})
export class DeliveryAddressPostalCityDataPipe implements PipeTransform {
    ...
    if (order) {
      ...
      if (deliveryAddress) {
        if (!deliveryAddress.useInvoiceAddress && deliveryAddress.address) {
          return displayAddressData(deliveryAddress.address, ['postal', 'city']);
          ...
          const address: Address =
            !order.invoice.forCompany && order.invoice.personalInvoice
              ...
              : null;
          return displayAddressData(address, ['postal', 'city']);
        }
      }
    }
