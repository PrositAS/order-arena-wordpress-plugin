...
export class OrdersService {
  constructor(
    ...
    this.bs
      ...
      .pipe(switchMap((createOrderByCustomer) => (createOrderByCustomer ? this.queryOrders() : of(null))))
      .subscribe();
  }
  ...
  getUniqueOrdersData(limit: number = null): Observable<OrderData[]> {
  ...
  // NOTE: might be useful later, need to check query
  // queryOrderRequests() {
  //   this.http
  //     .query<{ orderRequests: [] }>({
  ...
  //     .pipe(
  //       take(1),
  //       map((result) => result.data.orderRequests || [])
  //     )
  //     .subscribe();
  // }
}
