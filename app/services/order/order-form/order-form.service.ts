...
import { combineLatest, filter, map, Observable, switchMap, take, tap, zip } from 'rxjs';
...
export class OrderFormService {
  ...
  initOrderFormWithStoreData(orderForm: FormGroup<OrderForm>): Observable<void> {
    return combineLatest([
      ...
      filter(
        ([user, order, pastOrdersData, minDeliveryDate, minTimespanForDelivery, calendarClosedDates]) =>
          user &&
          order &&
          pastOrdersData &&
          minDeliveryDate &&
      ...
      tap(([user, order, pastOrdersData, minDeliveryDate, minTimespanForDelivery, calendarClosedDates]) => {
        if (orderForm) {
  ...
  calculateMaxGuests(): Observable<number> {
    return this.basketService.calculateMaxGuests();
  }
  ...
  completeOrder(order: Partial<Order>): Observable<any> {
    return zip([
  ...
  private fillAdditionalInfoForm(
    ...
    if (additionalInfo) {
      additionalInfoForm.reset(additionalInfo, { onlySelf, emitEvent });
    }
  }
}
