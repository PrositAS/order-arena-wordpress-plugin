...
export class OrderFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.user$
      .pipe(
        filter((user) => !!user),
        switchMap(() => this.orderFormService.initOrderFormWithStoreData(this.form).pipe(take(1)))
      )
      .subscribe();
    ...
    const deliveryTimeFormUpdated$ = this.deliveryTimeForm.valueChanges.pipe(
      ...
      map(([deliveryTime, minTimespanForDelivery]) => {
  ...
  completeOrder(): void {
    this.orderFormService
      ...
      .pipe(
        take(1),
        filter(
          (response) => (!response.errors || !response.errors?.length) && response.data.createOrderByCustomer
        ),
        tap(() => {
