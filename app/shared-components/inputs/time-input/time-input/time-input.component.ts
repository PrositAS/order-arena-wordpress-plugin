  ...
  implements AfterViewInit, OnDestroy, MatFormFieldControl<Time>, ControlValueAccessor
{
  ...
  get errorState(): boolean {
    return (
      (this.time.invalid && !!this.time.errors) ||
      (!!this.control?.touched && !!this.control?.invalid && !!this.control?.errors)
    );
  }
  ...
  onContainerClick() {
    if (this.hoursInput && this.minutesInput) {
      this._focusMonitor.focusVia(
        this.time.controls.hours.valid ? this.minutesInput : this.hoursInput,
        'program'
      );
    }
  }
